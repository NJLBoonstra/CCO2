...
func FindPalindromes(ctx context.Context, e job.GCSEvent) error {
	...
	err = job.UpdatePalindromeResult(fileName, myUUID, palindromes, longest_pal, fbClient, ctx)
	if err != nil {
	...
	allDone, _ := job.AllWorkerTypeStates(fileName, job.WorkerTypeState{Type: job.Palindrome, State: job.Completed}, fbClient, ctx)

	if allDone {
		// Last chunk, do something with merging perhaps

		res, _ := job.GetPalindromeResult(fileName, fbClient, ctx)
		longest := 0
		sum := 0
		for _, v := range res {
		...
		job.UpdatePalindromeJobResult(fileName, sum, longest, fbClient, ctx)
	}
